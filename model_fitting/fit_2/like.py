...
def luminosity_function(m, weights, lum_func):
    """
    measure the luminosity function in bins
    """
...
def lnlike(theta=None, y1=None, y1err=None, show_plots = False):
    """
    log-liklihood
    """
    ...
    if theta is None:
        gal_type='_centrals'
        d = {'M0':-20.44,
             'alpha':-1.00}  # use default model components' paramaters
    else:
        ...
        d = {'disk_shape_mu_1'+gal_type: theta[0],
             ...
             'morphology_m0'+gal_type: theta[8],
             'morphology_sigma'+gal_type: theta[9],
             'gamma_r'+gal_type: theta[10],
             'M0':theta[11],
             'alpha':theta[12]}
    ...
    # initialize intrinsic luminosity function
    # normalization does not matter here
    lum_func = MagSchechter(1.0, d['M0'], d['alpha'])

    # weights galaxies by intrinsic luminosity function
