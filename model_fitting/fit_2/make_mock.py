...
def make_galaxy_sample(mag_lim=-17, size=10**5, **kwargs):
    """
    Parameters
    ----------
    mag_lim : float
    ...
    # create tabel to store mock
    galaxy_table = Table()
    galaxy_table['gal_type'] = ['centrals']*size

    # uniformly sample galaxy luminosity rtange
    galaxy_table['Mag_r'] = -1.0*np.random.random(size=size)*(mag_lim - (-23.0)) + mag_lim

    # designate galaxies as disk or elliptical
    galaxy_table = morpholopgy_model.assign_morphology(table=galaxy_table)

    # assign orientation of galaxies
    galaxy_table = orientation_model.assign_orientation(table=galaxy_table)

    # assign 3d galaxy shape
